...
def svc_client_templates_creation(svc_client_with_templates):
    ...
    from renku.core.utils.requests import retry
    # ! TODO: import also METADATA when #1272 will be merged
    ...
    payload = {
        **template,
        ...
        'project_name': f'test-svc-{uuid.uuid4().hex}',
        'new_repo': 'https://dev.renku.ch/gitlab',
        'namespace': 'contact',
    }

    # ! TODO *** REMOVE ME ***
    authentication_headers['Renku-User-Id'
                           ] = '8925a18e056c4e53b37fcf11957fa433'
    authentication_headers['Renku-User-FullName'] = 'Lorenzo Bot'
    authentication_headers['Renku-User-Email'
                           ] = 'lorenzo.cavazzi.tech@gmail.com'
    authentication_headers['Authorization'] = 'Bearer gbQBpzQAjJdEy-ea-Xc-'
    payload['namespace'] = 'lorenzo.cavazzi.tech'
    # ! **********

    # clenup by invoking the GitLab delete API
    # TODO: use the project delete endpoint once implemented?
    def remove_project():
        project_slug = '{0}/{1}'.format(
            payload['namespace'], payload['project_name']
        )
        project_slug_encoded = urllib.parse.quote(project_slug, safe='')
        project_delete_url = '{0}/api/v4/projects/{1}'.format(
            payload['new_repo'], project_slug_encoded
        )
        with retry() as session:
